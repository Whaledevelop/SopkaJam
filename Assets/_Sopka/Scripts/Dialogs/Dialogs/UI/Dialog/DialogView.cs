...
namespace Whaledevelop.Dialogs.UI
{
    public class DialogView : UIView<DialogViewModel>
    {
        ...
        public override void Initialize()
        {
            ...
            DerivedModel.ChangeDownPanelCommand.Subscribe(OnChangeDownPanelCommand).AddToCollection(_subscriptions);
        ...
        private void OnChangeDownPanelCommand(bool status)
        {
            var isNarrator = string.IsNullOrEmpty(_speakerNameLabel.text);
            _textRootGameObject.SetActive(status && !isNarrator);
            _narratorTextRoot.SetActive(status && isNarrator);
        }
        ...
        private void OnChangeOptions()
        {
            ...
            if (DerivedModel.Options.Count > 0)
            {
                ...
                DerivedModel.MainText.Value = string.Empty;
                SetNextButtonEnabled(false);
            }
            else
            {
